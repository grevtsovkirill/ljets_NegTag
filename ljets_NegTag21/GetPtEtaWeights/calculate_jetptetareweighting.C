...
void DrawPlots(TString systematic, int ijet, TH2D* h2_mc, TH2D* h2_data, TString var)
{
  ...
  TH1D* h_mc=0;
  TH1D* h_data=0;
  if(var.Contains("eta"))  // To obtain the 1-D eta distr extract the Y axis  projection.
    {
  ...
  else if(var.Contains("pt")) // To obtain the 1-D pt distr extract the X axis  projection.
    {
  ...
  TString header;
  if (var.Contains("track")) headder = "leading track jet";
  else  header = "leading jet";
  if (ijet == 0) leg->SetHeader( header );
  else  leg->SetHeader("sub" + header );
...
void calculate_jetptetareweighting()
{
  ...
  TFile* ff_data = new TFile("data.root");
  TFile* ff_mc = new TFile("mc.root");
  const int njet = 2;
  TH2D* h_ratio[njet];
  ...
  for (auto prefix : prefixes ) {
    ...
    for (auto syst : systematics)
    {
      ...
      {
        TH2D* h_mc = (TH2D*)ff_mc->Get((string(syst + "/" + prefix + "pteta") + char('1' + i) + "_mc_w").c_str());
        TH2D* h_data = (TH2D*)ff_data->Get((string(syst + "/" + prefix + "pteta") + char('1' + i) + "_mc_w").c_str());
        if (!h_data) // necessary for data who only has the nominal branch.
        { // fall back to nominal branch
          h_data = (TH2D*)ff_data->Get((string("FlavourTagging_Nominal/" + prefix + "pteta") + char('1' + i) + "_mc_w").c_str());
          cout << "falling back to nominal data for: " << syst << endl;
        }
        // copiyng h_data in h_ratio[i] and divide it to h_mc.
        h_ratio[i] = (TH2D*)h_data->Clone((string(prefix + "ptscale") + char('1' + i)).c_str());
        ...
        DrawPlots(syst, i, h_mc, h_data, prefix + "eta");
        DrawPlots(syst, i, h_mc, h_data, prefix + "pt");
        delete h_mc;
        delete h_data;
        ...
        if (syst.EqualTo("FlavourTagging_Nominal"))
        {
          ...
          {
            // mcstat
            TH2D* h_mc_bootstrap_mc = (TH2D*)ff_mc->Get((string(syst + "/" + prefix + "pteta") + char('1' + i) + "_mc_w_" + to_string(ibootstrap)).c_str());
            TH2D* h_data = (TH2D*)ff_data->Get((string(syst + "/" + prefix + "pteta") + char('1' + i) + "_mc_w").c_str());
            // datastat
            TH2D* h_mc = (TH2D*)ff_mc->Get((string(syst + "/" + prefix + "pteta") + char('1' + i) + "_mc_w").c_str());
            TH2D* h_data_bootstrap = (TH2D*)ff_data->Get((string(syst + "/"+prefix+"pteta") + char('1' + i) + "_mc_w_" + to_string(ibootstrap)).c_str());

            // ratio
            h_ratio_bootstrap_mc[i][ibootstrap] = (TH2D*)h_data->Clone((string(prefix+"ptscale") + char('1' + i) + string("_mc_") + to_string(ibootstrap)).c_str());
            h_ratio_bootstrap_mc[i][ibootstrap]->Divide(h_mc_bootstrap_mc);

            h_ratio_bootstrap_data[i][ibootstrap] = (TH2D*)h_data_bootstrap->Clone((string(prefix+"ptscale") + char('1' + i) + string("_data_") + to_string(ibootstrap)).c_str());
