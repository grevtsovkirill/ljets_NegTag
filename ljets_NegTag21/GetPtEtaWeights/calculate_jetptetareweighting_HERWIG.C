...
void DrawPlots(TString systematic, int ijet, TH2D* h2_mc, TH2D* h2_data, TString var)
{
  ...
  TString header;
  if (var.Contains("track")) header = "leading track jet";
...
void calculate_jetptetareweighting_HERWIG()
{
  ...
  TFile* ff_mc = new TFile("mc_HERWIG.root");
  const int njet = 2;
  TH2D* h_ratio[njet];

  TFile* ff_out = new TFile("rew_HERWIG.root", "RECREATE");
  ...
  for (auto syst: systematics)
    {
      if(!syst.EqualTo("FlavourTagging_Nominal")) continue;
      cout << "####\n# working on: " << syst << "\n####" << endl;
