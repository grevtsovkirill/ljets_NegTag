...
void calculate_epsilon_HybEff(TH1D* Hists_incl,TH1D* Hists_wp, double& val){

  TH1* hist_incl = Hists_incl; // classic weighted histogram
  TH1* hist_wp = Hists_wp; // classic weighted histogram
  val = 0;

  double numerator = hist_wp->Integral(); 
  double denominator = hist_incl->Integral();

  //cout << "----> HybEff calc: numerator = " << numerator << ",  denominator = " << denominator << "; (h_incl = "<< hist_incl->GetMean() << ", h_wp = "<< hist_wp->GetMean()<< endl; 
...
int path_eps(string sfold="std", string compaigne="def", int bootstrap_bkeeper=0)
{
  ...
  map<int, map<int, map<string, map<string,map<string, TH1D*>>>>> h_rew_rewmode_pt_eta_f_m;
  TFile *f_MCbased;
  ...
  map<int, map<int, map<string, map<string, map<string, map<string, TH1D*>>>>>> h_mc;
  if(debug == 20) std::cout<< " Define h_mc map ; "<< std::endl;
  ...
  vector<string> hybMap={"","_Hyb60","_Hyb70","_Hyb77","_Hyb85"};

  for (string hyb_it: hybMap){   
  //for (unsigned i=0; i < hybMap1.size(); i++){   
    //cout << "JUST VECTOR   Name = "<< hyb_it<< endl;
  }
		...
		for (auto flav: flavors) {		  
		  ...
		  if( tagger=="MV2c10" && (sfold.find("d0smearing") != std::string::npos || 
					   ...
					   sfold.find("trackrecoeff") != std::string::npos ))
		    {
		      if(flav!="")
			{ 
			  if(debug == 20) std::cout<< "    flav not empty     : fill map with histo  "<<mode << "w_" << flav << "_" << p_eta.second << "_" << tagger<<hyb_it << std::endl;

			  h_mc[p_pt.first][p_eta.first][flav][tagger][mode][hyb_it] =
			    (TH1D*)(ff->Get((mode + "w_" + flav + "_" + p_eta.second + "_" + tagger+hyb_it).c_str()));
			  h_mc[p_pt.first][p_eta.first][flav][tagger][mode][hyb_it]->Multiply(h_rew_rewmode_pt_eta_f_m[p_pt.first][p_eta.first][flav][mode][hyb_it]);
			}
		      // inclusive comes after l,c and b have been computed
		      else if(flav=="")
			{
			  if(debug == 20) std::cout<< "    flav IS empty     : fill map with histo  "<<mode << "w_" << flav << "_" << p_eta.second << "_" << tagger<<hyb_it << std::endl;
			  
			  h_mc[p_pt.first][p_eta.first][flav][tagger][mode][hyb_it] = (TH1D*)h_mc[p_pt.first][p_eta.first]["l"][tagger][mode][hyb_it]->Clone((mode + "w_" + flav + "_" + p_eta.second + "_" + tagger+hyb_it).c_str());
			  h_mc[p_pt.first][p_eta.first][flav][tagger][mode][hyb_it]->Add(h_mc[p_pt.first][p_eta.first]["c"][tagger][mode][hyb_it]);
			  h_mc[p_pt.first][p_eta.first][flav][tagger][mode][hyb_it]->Add(h_mc[p_pt.first][p_eta.first]["b"][tagger][mode][hyb_it]);
			}
		    }     
		  // HERWIG ++ inclusive -> same HF than Pythia
		  else if( sfold.find("generator")!= std::string::npos && flav=="")
		    {
		      ...
		      double Ntot = h_mc[p_pt.first][p_eta.first]["l"][tagger][mode][hyb_it]->Integral() + h_mc[p_pt.first][p_eta.first]["c"][tagger][mode][hyb_it]->Integral() + h_mc[p_pt.first][p_eta.first]["b"][tagger][mode][hyb_it]->Integral();
		      // inclusive HERWIG histo with Pythia HF fractions
		      h_mc[p_pt.first][p_eta.first][flav][tagger][mode][hyb_it] = (TH1D*)h_mc[p_pt.first][p_eta.first]["c"][tagger][mode][hyb_it]->Clone((mode + "w_" + flav + "_" + p_eta.second + "_" + tagger+hyb_it).c_str());
		      h_mc[p_pt.first][p_eta.first][flav][tagger][mode][hyb_it]->Scale(h_fracc_Pythia->GetBinContent(ipt, ieta)*Ntot/h_mc[p_pt.first][p_eta.first][flav][tagger][mode][hyb_it]->Integral());
		      h_mc[p_pt.first][p_eta.first][flav][tagger][mode][hyb_it]->Add(h_mc[p_pt.first][p_eta.first]["b"][tagger][mode][hyb_it], h_fracb_Pythia->GetBinContent(ipt, ieta)*Ntot/h_mc[p_pt.first][p_eta.first]["b"][tagger][mode][hyb_it]->Integral());
		      h_mc[p_pt.first][p_eta.first][flav][tagger][mode][hyb_it]->Add(h_mc[p_pt.first][p_eta.first]["l"][tagger][mode][hyb_it], (1-h_fracb_Pythia->GetBinContent(ipt, ieta)-h_fracc_Pythia->GetBinContent(ipt, ieta))*Ntot/h_mc[p_pt.first][p_eta.first]["l"][tagger][mode][hyb_it]->Integral());
		    }
		  // regular case
		  else{
		    h_mc[p_pt.first][p_eta.first][flav][tagger][mode][hyb_it] = (TH1D*)(ff->Get((mode + "w_" + flav + "_" + p_eta.second + "_" + tagger+hyb_it).c_str()));
		  }
		} // end loop flavors
	      }// hybrid wp loop
	    }// mode loop
  ...
  // bootstrap
  // [ipt] [ieta] [tagger]
  map<int, map<int, map<string, map<string, TH1D*>>>> h_data_neg;
  ...
  if(sfold.find("datastat")==std::string::npos)
  {
    ...
    for (auto tagger: conf::tagger_list) {
      for (auto p_pt: kin_labels) {
        for (auto p_eta: p_pt.second){
	  for (auto hyb_it: hybMap){   
            h_data_neg[p_pt.first][p_eta.first][tagger][hyb_it] = 
              (TH1D*)(ff_data->Get(("negw__" + p_eta.second + "_" + tagger+hyb_it ).c_str()));
	  }
        }
      }
    }
  ...
  if(sfold=="FlavourTagging_Nominal" || sfold=="subleadingjet")
  {
    ...
    if(debug == 20 )cout << "@@@ read data_fraction_path"<< endl;
    ...
    if(debug == 20 )cout << "@@@ define hists"<< endl;
...
*/
    ...
    if(debug == 20 )    cout << "@@@ read mc_fraction_path"<< endl;
    ...
    for (auto tagger: conf::tagger_list){
	  ...
	  for (auto hyb_it: hybMap){
	    ...
	    if(debug == 20 )cout << "@@@ in loop tagger/pt/eta "<< endl;

	    string partial_identifier = p_eta.second + "_" + tagger;
	    if(debug == 20 )cout << "@@@ partial_identifier "<< partial_identifier<< endl;
	    
	    TH1D* h_negl = h_mc[p_pt.first][p_eta.first]["l"][tagger]["neg"][hyb_it];
	    TH1D* h_negc = h_mc[p_pt.first][p_eta.first]["c"][tagger]["neg"][hyb_it];
	    TH1D* h_negb = h_mc[p_pt.first][p_eta.first]["b"][tagger]["neg"][hyb_it];
	    ...
	    */
	    ...
	    //if(sl!=0 && sc!=0 && sb!=0){
	    h_mc[p_pt.first][p_eta.first]["HFv"][tagger]["neg"][hyb_it] = h_negHFv;
	    ...
	    TH1D* h_data_neg_HFsub = (TH1D*)h_data_neg[p_pt.first][p_eta.first][tagger][hyb_it]->Clone((string("HFsub_")+partial_identifier).c_str());
	    h_mc[p_pt.first][p_eta.first]["data_HFsub"][tagger]["neg"][hyb_it] = h_data_neg_HFsub; 
  ...
  map<std::string,  map<std::string, map<double, map<int, map< std::string, TH1D*>>>>> h_plots;

  if(debug == 20)   std::cout << "PLOTS vs pT as a function of [tagger][wp][ieta][prefix]"  << std::endl;
  ...
  for (auto tagger: conf::tagger_list){
    ...
    for (auto wp: wpoint_cuts){
      ...
      if(debug == 20) std::cout << "Loop tagger/wpoint_cut ; tagger: " << tagger  << " WP: " << wp_label<< std::endl;
	...
	for (auto prefix: plot_prefixes){
	  for (auto wp_type_i: wp_type_prefixes){
	    ...
	    string identifier = prefix + "_" + tagger + "_w"+wp_type_i + wp_label + "_eta" + to_string(ieta);
	    ...
	    string title = tagger + " "+wp_type_i+" WP = " + wp_label + "%, eta region " + eta_title;
	    h_plots[tagger][wp_type_i][wp][ieta][prefix] = new TH1D(identifier.c_str(), title.c_str(), conf::n_pt, conf::pt_lowedges);
	  }//type of WP - FixedCut (FC) / HybEff (HE) - to be add PseudoContinious (PC)
	}//prefix
      }//eta loop
      ++iwp;
    }//wp loop
  ...
  if(debug == 20) std::cout<< "calculate all histograms"<< std::endl;	    
  for (auto tagger: conf::tagger_list){
	...
	for (auto p_eta: p_pt.second){
	  for (auto wp_type_i: wp_type_prefixes){
	    auto cur_histo = h_plots[tagger][wp_type_i][wp][p_eta.first];
	    ...
	    if(debug == 20) cout<< "check nonull "<< h_mc[p_pt.first][p_eta.first]["l"][tagger][""][""]<< endl;
	    //if(h_mc[p_pt.first][p_eta.first]["l"][tagger][hyb_it]==0|| h_mc[p_pt.first][p_eta.first]["l"][tagger]["neg"]==0 || h_mc[p_pt.first][p_eta.first][hyb_it][tagger]["neg"]==0) continue;
		...
		if(wp_type_i=="FC"){
		  ...
		  if(debug == 200) std::cout<< tagger<<" at wp = "<< wp <<" (iwp= "<< iwp<<") eff hist_mc : calculate_epsilon_incl "<< eps_a_neg <<" ("<< p_pt.first <<","<< p_eta.first<< ")" <<std::endl;	    
		}
		
		//HybEffWP
		if(wp_type_i=="Hyb"){
		  calculate_epsilon_HybEff(h_mc[p_pt.first][p_eta.first]["l"][tagger][""][""],h_mc[p_pt.first][p_eta.first]["l"][tagger][""][hybMap[iwp+1]], eps_l);
		  calculate_epsilon_HybEff(h_mc[p_pt.first][p_eta.first]["l"][tagger]["neg"][""],h_mc[p_pt.first][p_eta.first]["l"][tagger]["neg"][hybMap[iwp+1]], eps_l_neg);
		  calculate_epsilon_HybEff(h_mc[p_pt.first][p_eta.first][""][tagger]["neg"][""],h_mc[p_pt.first][p_eta.first][""][tagger]["neg"][hybMap[iwp+1]], eps_a_neg);
		  if(debug == 200) std::cout<< "                 : calculate_epsilon_HybEff "<< hybMap[iwp+1] <<" "<< eps_a_neg<<std::endl;	    
		}

	      }
	    
	    if(debug == 20) std::cout<< "Status of cut_histo: tagger = " << tagger << "_"<< wp << " " << p_pt.first << " "<< p_eta.first<< ": eps_l= "<< eps_l << "; eps_l_neg= "<< eps_l_neg << "; eps_a_neg= "<< eps_a_neg<<std::endl;	  
	    ...
	    if(sfold.find("datastat")!=std::string::npos) calculate_epsilon(h_data_neg_bootstrap_data[p_pt.first][p_eta.first][tagger], eps_neg_d, wp);
	      ...
	      if(wp_type_i=="FC")  calculate_epsilon(h_data_neg[p_pt.first][p_eta.first][tagger][""], eps_neg_d, wp);
	      if(wp_type_i=="Hyb") calculate_epsilon_HybEff(h_data_neg[p_pt.first][p_eta.first][tagger][""],h_data_neg[p_pt.first][p_eta.first][tagger][hybMap[iwp+1]], eps_neg_d);
	    }
	    ...
	    if(sfold=="notrackrew" || sfold=="notrackrew_subleadingjet"){
	      if(debug == 20) std::cout << " ---compute SF_Nominal:  notrackrew*, eps_l_nominal = "<< eps_l<< std::endl;
	      eps_l_NOMINAL = eps_l;
	    }
	    else
	      {
	    ...
	    if(sfold=="notrackrew" || sfold=="notrackrew_subleadingjet"){
	      ...
	      if(debug == 20) std::cout <<"--- compute SF Neg: in notrackrew*, set eps_l_neg_NOMINAL = eps_l_neg:  "<<  eps_l_neg_NOMINAL << std::endl;
	    }
	    else
	      {
		...
		if(debug == 20 )std::cout << "@@@ not notrackrew "<<histo_name << std::endl;
		...
		f_ntrack_neg->Close();
		if(debug == 20) std::cout <<" ---compute SF Neg:  REWEIGHTED  f_ntrack_neg = "<< f_ntrack_neg_name<< "; grab hist: "<< histo_name << ";  eps_l_neg_NOMINAL = h_eps_l_neg_ntrack->GetBinContent(pt)="<< eps_l_neg_NOMINAL << std::endl;
	      }
	    //*/
	    ...
	    if(debug == 20) std::cout <<"--- compute SF Neg: general, eps_l_neg = "<<eps_l_neg <<"(eps_l_NOMINAL = "<<eps_l_neg_NOMINAL<< "),  eps_neg_d = "<< eps_neg_d<< "; khf = "<< khf << ";  eps_l_neg_NOMINAL ="<< eps_l_neg_NOMINAL<< "; sf_neg = eps_neg_d*khf/eps_l_neg_NOMINAL = "<< sf_neg  << std::endl;
		...
		if(debug == 20) cout<< "check nonull: effect of variation of b/c jet fractions in eps_a_neg: determined from data.  "<< h_mc[p_pt.first][p_eta.first]["l"][tagger][""][""]<< endl;
		...
		if(debug == 20) cout<< "estimate HF for  "<< wp_type_i<< " = " << endl;
		if(wp_type_i=="FC") calculate_epsilon(h_mc[p_pt.first][p_eta.first]["HFv"][tagger]["neg"][""], eps_a_neg_HF, wp);
			
		if(wp_type_i=="Hyb") calculate_epsilon_HybEff(h_mc[p_pt.first][p_eta.first]["HFv"][tagger]["neg"][""],h_mc[p_pt.first][p_eta.first]["HFv"][tagger]["neg"][hybMap[iwp+1]], eps_a_neg_HF);
		if(debug == 20) cout<< "                        "<< eps_a_neg_HF<< " = " << endl;
		...
		double eps_c_neg = 0;
		double eps_b_neg = 0;
		if(debug == 20) cout<< "estimate eps_c_neg and eps_c_neg  for  "<< wp_type_i<< " = " << endl;
		...
		if(wp_type_i=="Hyb"){
		  calculate_epsilon_HybEff(h_mc[p_pt.first][p_eta.first]["c"][tagger]["neg"][""], h_mc[p_pt.first][p_eta.first]["c"][tagger]["neg"][hybMap[iwp+1]], eps_c_neg);
		  calculate_epsilon_HybEff(h_mc[p_pt.first][p_eta.first]["b"][tagger]["neg"][""], h_mc[p_pt.first][p_eta.first]["b"][tagger]["neg"][hybMap[iwp+1]], eps_b_neg);
		}
		if(debug == 20) cout<< "           eps_c_neg  "<< eps_c_neg<< " , eps_b_neg = " << eps_b_neg<<endl;
		if(debug == 20) cout<< " read h_neg_I_ l/c/b  "<< endl;
		...
		if(wp_type_i=="FC") calculate_epsilon(h_mc[p_pt.first][p_eta.first]["data_HFsub"][tagger]["neg"][""], eps_neg_d_HFsub, wp);
		if(wp_type_i=="Hyb") calculate_epsilon_HybEff(h_mc[p_pt.first][p_eta.first]["data_HFsub"][tagger]["neg"][""], h_mc[p_pt.first][p_eta.first]["data_HFsub"][tagger]["neg"][hybMap[iwp+1]], eps_neg_d_HFsub);
	  ...
	  }//loop over wp type fixed/hybrid - to be added pseudo continious
	}// loop over eta
      }// loop over pt
      iwp++;
    }// loop over WPs
  }// loop over taggers
  if(debug == 20) std::cout <<" ----- quite loop over taggers" << std::endl;
  ...
  if(debug == 20) std::cout <<"  Towards output" << std::endl;
  ...
  for (auto tagger: conf::tagger_list){
	...
	for (auto prefix: plot_prefixes){
	  for (auto wp_type_i: wp_type_prefixes){
	    if(debug == 20) std::cout <<"  about to write tagger_wp_ieta_prefix" << tagger << "_" << wp << "_" << ieta << "_" << prefix << std::endl;
	    
	    h_plots[tagger][wp_type_i][wp][ieta][prefix]->Write();
	  }
	}
      }
    }
  ...
  if(debug == 20) std::cout <<"  deleting - h_plots " << std::endl;
  ...
  for (auto tagger: conf::tagger_list){
	...
	for (auto prefix: plot_prefixes){
	  for (auto wp_type_i: wp_type_prefixes){
	    delete h_plots[tagger][wp_type_i][wp][ieta][prefix];
	  }
	}
      }
    }
  ...
  if(debug == 20) std::cout <<"  deleting - h_mc " << std::endl;

  for(auto const& ent1 : h_mc) 
  {
    if(debug == 20) std::cout <<"              h_mc:ent1 "<< ent1.first << std::endl;
    for(auto const& ent2 : ent1.second) 
    {
      if(debug == 20) std::cout <<"              h_mc:ent2 "<< ent2.first << std::endl;
      for(auto const& ent3 : ent2.second) 
      {
	if(debug == 20) std::cout <<"              h_mc:ent3 "<< ent3.first << std::endl;
        for(auto const& ent4 : ent3.second) 
        {
	  if(debug == 20) std::cout <<"              h_mc:ent4 "<< ent4.first << std::endl;
    
          for(auto const& ent5 : ent4.second) 
	    {
	      if(debug == 20) std::cout <<"              h_mc:ent5 "<< ent5.first << std::endl;

	      //for(auto const& ent6 : ent5.second){
	      //if(debug == 20) std::cout <<"              h_mc:ent6 "<< ent6.first << std::endl;

	      //delete h_mc[ent1.first][ent2.first][ent3.first][ent4.first][ent5.first][ent6.first]; 
	      delete h_mc[ent1.first][ent2.first][ent3.first][ent4.first][ent5.first][""]; 
		    //}
	    }
        }
      }
  ...
  if(debug == 20) std::cout <<"  deleting - start deleting bootstraps " << std::endl;
  ...
  if(debug == 20) std::cout <<"  deleting - h_data_neg " << std::endl;
  ...
  if(debug == 20) std::cout <<"  ff->Close  " << std::endl;
  ...
  if(debug == 20) std::cout <<"  clear h_*  " << std::endl;
