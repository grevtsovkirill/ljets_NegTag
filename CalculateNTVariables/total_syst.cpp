...
int main(int argc, char* argv[]) {
  ...
  for (auto tagger: conf::tagger_list){
	...
	for (auto wp_type_i: wp_type_prefixes){
	  std::cout << "*************** WP Type " << wp_type_i << "***************" << std::endl;
	  
	  
	  string partial_identifier = "_" + tagger + "_w"+wp_type_i + wp_label + "_eta" + to_string(ieta);
	  std::cout << "-------- partial_identifier = "<<partial_identifier<< endl;
	  ...
	  for (int ipt=0; ipt<conf::n_pt; ++ipt){
		...
		{
		  cout << partial_identifier<<endl;
		  if(ipt==0)
		    {
		      //test=vars[i];
		      //vars[i]="sf";
		      TH1* hstat = (TH1*)sys_files[systematics[i]]->Get("rel_" + vars[i] + partial_identifier);
		      //TH1* hstat = (TH1*)sys_files[systematics[i]]->Get("rel_sf" + partial_identifier);
		      if (!hstat)
			cout << " STATISTICAL UNCERTAINTY subleading ----> no histogram found for " << systematics[i] << ", looking for rel_" << vars[i]<< partial_identifier << endl;
		  ...
		  if(ipt!=0)
		    {
		      ...
		      //TH1* hstat2 = (TH1*)sys_files[systematics[i]]->Get("rel_sf" + partial_identifier);
		      if (!hstat2)
		      ...
		      (systematics.at(i).Contains("FlavourTagging") && !systematics.at(i).Contains("FlavourTagging_JET_JER_SINGLE_NP__1up")) && !systematics.at(i).Contains("FlavourTagging_JET_JvtEfficiency__1up") )
		{
		  ...
		  if (!hsys_up || !hsys_down){ cout << "SYSTEMATIC UNCERTAINTY ----> no histogram found for " << systematics[i] << endl;
		    continue;
		  }
