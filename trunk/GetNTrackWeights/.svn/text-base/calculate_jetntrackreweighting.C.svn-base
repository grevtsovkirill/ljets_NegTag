	  ...
	  hratio[ij]->Write();
          ...
          {   
            for(int ibootstrap=0; ibootstrap<conf::n_bootstrap; ibootstrap++)
            {
              ...
              TH2D *hmc_bootstrap_mc = (TH2D*)f_mc->Get( (string(syst) + string("/ntrk_pt") + char('1'+ij)+ string("_pteta_rew_mc_")+to_string(ibootstrap)).c_str()); 

              //datastat
              // for data, data bootstrap weight applied
              TH2D *hdata_bootstrap_data = (TH2D*)f_da->Get( (string(syst) + string("/ntrk_pt") + char('1'+ij)+ string("_pteta_rew_data_")+to_string(ibootstrap)).c_str()); 
