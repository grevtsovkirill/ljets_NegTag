	  ...
	  if(isleading[j] && jetpass_leading==0 && jetpass_subleading<2 )
		...
		if(trackisleading[j]){
			...
			k=0; 	
		        ...
		        h_trackpt2_mc_w->Fill(trackptj, weight);
	                h_tracketa2_mc_w->Fill(tracketa_abs, weight);
        	        h_trackpt_thin2_mc_w->Fill(trackptj, weight);
                	h_tracketa_thin2_mc_w->Fill(tracketa_abs, weight);
                	h_trackphi2_mc_w->Fill(trackjetphi[j], weight);
	                k=0;
                	for (const auto &name: subtagger::floats_trackjet){
                  	  h_float_subtagger2_track_mc_w.at(k)->Fill(float_subtagger_trackjet[name.first][j], weight);
                  	  k++;
                }

	                k=0;
        	        for (const auto &name: subtagger::ints_trackjet){
                	  h_int_subtagger2_track_mc_w.at(k)->Fill(int_subtagger_trackjet[name.first][j], weight);
                	  k++;	
                }

                	k=0;
                	for (const auto &name: subtagger::doubles_trackjet){
                	  h_double_subtagger2_track_mc_w.at(k)->Fill(double_subtagger_trackjet[name.first][j], weight);
                	  k++;
                }

	                h_trackntrk_IP3DNeg2_mc_w->Fill(trackntracks_IP3DNeg, weight);
        	        h_trackntrk_IP3D2_mc_w->Fill(trackntracks_IP3D, weight);
                	h_trackntrk_IP2DNeg2_mc_w->Fill(trackntracks_IP2DNeg, weight);
                	h_trackntrk_IP2D2_mc_w->Fill(trackntracks_IP2D, weight);
                	h_trackntrk_SV1Flip2_mc_w->Fill(trackntracks_SV1Flip, weight);
                	h_trackntrk_SV12_mc_w->Fill(trackntracks_SV1, weight);
                	h_trackntrk_JetFitterFlip2_mc_w->Fill(trackntracks_JetFitterFlip, weight);
                	h_trackntrk_JetFitter2_mc_w->Fill(trackntracks_JetFitter, weight);

	                h_trackpteta2_mc_w->Fill(trackptj, tracketa_abs, weight);

                	h_trackntrk_IP3DNeg_pt2_mc_w->Fill(trackntracks_IP3DNeg, trackptj, weight);
                	h_trackntrk_IP3D_pt2_mc_w->Fill(trackntracks_IP3D, trackptj, weight);
                	h_trackntrk_IP2DNeg_pt2_mc_w->Fill(trackntracks_IP2DNeg, trackptj, weight);
                	h_trackntrk_IP2D_pt2_mc_w->Fill(trackntracks_IP2D, trackptj, weight);
                	h_trackntrk_SV1Flip_pt2_mc_w->Fill(trackntracks_SV1Flip, trackptj, weight);
                	h_trackntrk_SV1_pt2_mc_w->Fill(trackntracks_SV1, trackptj, weight);
        	        h_trackntrk_JetFitterFlip_pt2_mc_w->Fill(trackntracks_JetFitterFlip, trackptj, weight);
	                h_trackntrk_JetFitter_pt2_mc_w->Fill(trackntracks_JetFitter, trackptj, weight);
		}
			...
			array_h_trackpteta2_mc_w[ibootstrap]->Fill(trackptj, tracketa_abs, weight*weight_bootstrap[(bootstrap_bkeeper-1)*2+ibootstrap]);
			}
		}
	      }
