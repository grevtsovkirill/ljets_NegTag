  ...
  for (auto tagger: conf::tagger_list){
	...
	for (int ipt=0; ipt<conf::n_pt; ++ipt){
	  ...
	  h_out_tot_down[partial_identifier]->SetBinContent(ipt+1, -sqrt(tot_stat*tot_stat+tot_syst_down*tot_syst_down)); // total relative !!
        ...
        pt->SetTextSize(0.04);
        pt->SetTextFont(42);
        std::string eta_region = "central";
        if(ieta==2) eta_region = "forward";
        pt->AddText("#sqrt{s} = 13 TeV, 3.2 + 32.9 fb ^{-1}");
        pt->AddText((tagger + string(", #epsilon_{b} = ") + wp_label + string("%    ") +  string("|#eta^{jet}| ") + eta_region).c_str());
